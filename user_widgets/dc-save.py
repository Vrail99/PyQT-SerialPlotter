...
class DensityWidget(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._last_publish_update_time = 0.0
    ...
    def save_info_to_file(self, system, result, deltaP, flow, std_rho) -> None:
        output_dict = {
            "Flow (m3/s)": flow,
            "Delta P (Pa)": deltaP,
    ...
    def processSample(self, timestamp: float, values: list[float]) -> None:
        ...
        # Accumulate dP samples for averaging
        dp_mbar = float(values[dp_idx])
        self._dp_sample_buffer.append(dp_mbar)

        # Throttle updates to once per second to avoid hanging the calculator
        current_time = time.time()
        if current_time - self._last_publish_update_time < 1.0:
            return
        self._last_publish_update_time = current_time

        # Use average of accumulated dP samples
        if self._dp_sample_buffer:
            avg_dp_mbar = sum(self._dp_sample_buffer) / len(self._dp_sample_buffer)
            self._dp_sample_buffer = []
            self._update_result_from_dp(avg_dp_mbar, source="sample", emit_output=True)
    ...
    def _update_result_from_dp(self, dp_mbar: float, source: str, emit_output: bool = False) -> None:
        ...
        try:
            ...
            if emit_output:
                self.derivedSampleReady.emit(self.output_channel_spin.value(), std_rho)
        except Exception as exc:
            self.status_label.setText(f"Status: calculation error ({exc})")

    def _build_system(self) -> GasMeasurementSystem:
        """
        Constructs a GasMeasurementSystem instance based on user input parameters.
