"""
Parameter tree management for SerialPlotter.

Owns the ParameterTree widget, handles all parameter change events,
and provides update methods for live statistics and sample rate display.
"""
...
class ParameterManager(QObject):
    ...
    Internalises all parameter-change dispatch so SerialPlotter
    does not need to know about individual parameter names.
    """

    def __init__(self, config, plot_manager, data_buffer,
                 fft_calculator, data_processor, file_stream_manager):
        super().__init__()
        self.config = config
    ...
    def setup(self, param_tree: ParameterTree) -> Parameter:
        """Build the parameter tree, attach it to the widget, return the root."""
        stats_params = [
            {
                'name': line_cfg.name,
                'type': 'group',
    ...
    def update_statistics(self, stats_list) -> None:
        """Push new statistics values into the parameter tree."""
    ...
    def update_fs(self, rate: float) -> None:
        """Update the displayed sample rate."""
        if self.params:
